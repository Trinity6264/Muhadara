...
import 'package:muhadara/views/home/trending/trending_view_model.dart';
...
class TrendingView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ViewModelBuilder<HomeSubViewModel>.reactive(
      onModelReady: (model) => model.fecthAll(1),
      ...
      builder: (BuildContext contxt, HomeSubViewModel model, Widget? child) {
        ...
        return model.isBusy
            ? const ShimmerEffect()
            : model.trendingData!.isEmpty
                ? const SizedBox.shrink()
                : SizedBox(
                    height: _size.height * 0.25,
                    child: ListView.builder(
                      itemCount: model.trendingData?.length,
